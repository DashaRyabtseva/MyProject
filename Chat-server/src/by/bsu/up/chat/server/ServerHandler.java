...
public class ServerHandler implements HttpHandler {
    ...
    private Response doGet(HttpExchange httpExchange) {
        String query = httpExchange.getRequestURI().getQuery(); //получили наш запрос в каком-то норм виде
        if (query == null) { // если он так себе
            return Response.badRequest("Absent query in request");
        }
        Map<String, String> map = queryToMap(query); // НЕ ЯСНО(
        String token = map.get(Constants.REQUEST_PARAM_TOKEN); // эта константа =='token'
